...
def distribuicao_idades(df):
    ...
    # Labels e ticks
    plt.xlabel(
        "Idade", color="#E2CC9C", fontsize=14
    )
    plt.ylabel(
        "Densidade", color="#E2CC9C", fontsize=14
    )
    plt.xticks(color="#E2CC9C", fontsize=10)
    plt.yticks(color="#E2CC9C", fontsize=10)
...
def satisfacao_plano(df):
    ...
    media_satisfacao = df.groupby(
        "membership_type"
    )["satisfaction_score"].mean()

    # Definido o tamanaho do gráfico
    plt.figure(
        figsize=(10, 6), facecolor="#0A2538"
    )
...
def activity_category(df):
    ...
    # Add title and labels
    plt.title(
        "Alunos por categoria de atividade e tipo de membro",
        color="#E84A2E",
        fontsize=16,
    )
    plt.xlabel(
...
def distribuicao_assinaturas(df):
    ...
    for text in texts:
        text.set_color(
            "#E84A2E"
        )  # Define a cor "#E84A2E" para os nomes dos planos

    for autotext in autotexts:
        autotext.set_color(
            "Black"
        )  # Define o texto do percentual em branco
